...
def test_main_passes_leading_curl_flags_to_wrapper(tmp_path):
    ...
    wrapper.will_intercept.return_value = False
    logger = MagicMock()

    with (
        patch("curlguard.cli.load_config", return_value=config),
    ...
    assert exit_code == 0
    wrapper.dispatch.assert_called_once_with(
...
def test_shim_preserves_real_curl_version_behavior(tmp_path, monkeypatch):
    config = MagicMock()
    config.rules_dirs = []
    ...
    with (
        patch("curlguard.cli.load_config", return_value=config),
        patch("curlguard.cli.YaraScanner"),
        patch("curlguard.cli.AutoUpdater"),
        patch("curlguard.cli.AuditLogger", return_value=logger),
        patch("curlguard.cli.SslBypassDetector"),
        patch("curlguard.cli.CurlWrapper", return_value=wrapper),
    ):
    ...
    assert exit_code == 0
    wrapper.dispatch.assert_called_once_with(["--version"])
    logger.close.assert_called_once()
